//+------------------------------------------------------------------+
//|              XAUUSDm_HWR_LondonNY_Full.mq5                        |
//|   High Winrate + London-NY Session (WIB GMT+7 via TimeGMT)        |
//|   TF Entry: M5, HTF Confirm: M15                                  |
...
//====================== INPUT ======================================
input int             InpMagic              = 2026012403;
input ENUM_TIMEFRAMES InpTF                 = PERIOD_M5;   // Entry TF
input ENUM_TIMEFRAMES InpHTF                = PERIOD_M15;  // Confirm TF

// --- Time filter (WIB, GMT+7) - London + NY by default
input bool   InpUseTimeFilterWIB   = true;
input int    InpStartHourWIB       = 15;   // London open WIB ~15:00
input int    InpStartMinuteWIB     = 0;
input int    InpEndHourWIB         = 4;    // NY runs until ~04:00 WIB
input int    InpEndMinuteWIB       = 0;    // End is EXCLUSIVE
input bool   InpTradeMonToFriOnly  = true;

// --- HWR Strategy (tight filters)
input int    InpEmaFast            = 20;
input int    InpEmaSlow            = 50;
input int    InpRSIPeriod          = 14;
input double InpRSI_BuyMin         = 55.0;  // tighter => higher winrate
input double InpRSI_SellMax        = 45.0;

input int    InpPullbackMaxPoints  = 350;   // tighter pullback
input int    InpCooldownBars       = 4;     // less trading

// --- Filter 2: Range (tight)
input int    InpMinTrendPoints     = 260;   // EMA distance must be significant

// --- Filter 3: Impulse (tight)
input double InpImpulseATRMult     = 1.35;  // skip spike candles

// --- SL/TP
input bool   InpUseATRStops        = true;
input int    InpATRPeriod          = 14;
input double InpATR_SL_Mult        = 1.35;
input double InpATR_TP_Mult        = 1.20;

input int    InpSL_Points          = 1300;  // if ATR off
input int    InpTP_Points          = 1600;  // if ATR off
...
input int    InpMaxSpreadPoints    = 250;   // spread kamu 160 => aman
...
input double InpTP1_ATR_Mult        = 0.80;  // TP1 at 0.8 ATR
input int    InpTP1_Points          = 900;   // if ATR off
...
bool InTradingWindowWIB()
{
   ...
   // Crossing midnight window (e.g., 15:00 -> 04:00)
   return (curMin >= startMin || curMin < endMin);
}

//====================== FILTERS (1..3) =============================

// (1) HTF trend confirm (closed HTF bar)
...
// (3) Impulse filter (skip if prev candle too big)
bool NoImpulse()
{
   double atr[1];
...
int OnInit()
{
   ...
   Print("HWR London-NY ready on ", _Symbol,
...
void OnTick()
{
   ...
   // Only trade on new bar to keep HWR nature
   if(!IsNewBar()) return;
   ...
   bool ok=false;
   if(sig==1) ok=trade.Buy(lot,_Symbol,0.0,sl,tp,"HWR LNY Buy");
   else       ok=trade.Sell(lot,_Symbol,0.0,sl,tp,"HWR LNY Sell");
