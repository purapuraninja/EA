//+------------------------------------------------------------------+
//|          XAUUSDm_Frequent_Aggressive_Full.mq5                     |
//|   Frequent (Aggressive) Scalping: M5 Entry + M15 Confirm          |
//|   Filters: (1) HTF Trend, (2) Range, (3) Impulse, (4) StopSafety  |
...
//====================== INPUT ======================================
input int             InpMagic              = 2026012401;
input ENUM_TIMEFRAMES InpTF                 = PERIOD_M5;   // Entry TF
input ENUM_TIMEFRAMES InpHTF                = PERIOD_M15;  // Confirm TF

// --- Core strategy (AGGRESSIVE)
input int    InpEmaFast            = 20;
input int    InpEmaSlow            = 50;
input int    InpRSIPeriod          = 14;
input double InpRSI_BuyMin         = 45.0;   // aggressive
input double InpRSI_SellMax        = 55.0;   // aggressive

input int    InpPullbackMaxPoints  = 2500;   // aggressive: allow far pullback (gold often runs)
input int    InpCooldownBars       = 0;      // aggressive: no cooldown

// --- Filter (2) Range filter (AGGRESSIVE)
input int    InpMinTrendPoints     = 20;     // aggressive: allow early trend / light separation

// --- Filter (3) Impulse filter (AGGRESSIVE)
input double InpImpulseATRMult     = 6.0;    // aggressive: tolerate spikes more
...
input int    InpSL_Points          = 1300;   // used if ATR off
input int    InpTP_Points          = 1800;   // used if ATR off

// --- Compounding (optional)
input bool   InpUseCompoundStep    = true;
input double InpBaseBalance        = 100.0;
input double InpBaseLot            = 0.01;
input double InpStepBalance        = 50.0;   // every +$50
input double InpLotStep            = 0.01;   // add +0.01 lot
input double InpMinLot             = 0.01;
input double InpMaxLot             = 0.30;

// --- Protection / execution
input int    InpMaxSpreadPoints    = 400;    // aggressive but still safe (spread kamu ~160)
input int    InpDeviationPoints    = 80;
input bool   InpOnePositionOnly    = true;

// --- Break-even
input bool   InpUseBreakEven       = true;
input int    InpBE_StartPoints     = 700;    // aggressive: earlier BE
input int    InpBE_OffsetPoints    = 50;

// --- Trailing
input bool   InpUseTrailing        = true;
input int    InpTrailStartPoints   = 900;
input int    InpTrailDistance      = 650;
input int    InpTrailStep          = 50;

// --- Partial Close TP1
input bool   InpUsePartialTP1       = true;
input double InpTP1_ClosePercent    = 50.0;  // close 50% at TP1
input bool   InpTP1_UseATR          = true;  // TP1 based on ATR
input double InpTP1_ATR_Mult        = 0.7;   // aggressive: TP1 closer
input int    InpTP1_Points          = 700;   // if ATR off
input bool   InpTP1_MoveSLToBE      = true;  // move SL to BE after TP1
input int    InpTP1_BE_OffsetPoints = 30;
...
// (1) HTF trend confirm (use closed bar)
...
// (3) Impulse filter (skip if previous candle range too large)
bool NoImpulse()
{
   double atr[1];
...
bool ApplyStopSafety(bool buy, double entryPrice, double &sl, double &tp)
{
   ...
   int minPts = MathMax(stopsLevel, freezeLevel) + 8; // buffer
...
int GetSignal()
{
   ...
   // Pullback distance to EMA fast (aggressive allowed far)
   ...
   for(int i=total-1;i>=0;i--)
   {
      ...
      if(InpUsePartialTP1)
      {
         ...
         if(!tp1Done && tp1Pts > 0 && profitPts >= tp1Pts)
         {
            ...
            {
               // can't partial due to min lot; mark done to avoid repeated attempts
               GlobalVariableSet(flag, TimeCurrent());
            }
         }
      }
...
int OnInit()
{
   ...
   Print("Frequent AGGRESSIVE ready on ", _Symbol,
...
void OnTick()
{
   ...
   // trade only on new bar to avoid over-trading in the same candle
   if(!IsNewBar()) return;
   ...
   bool ok=false;
   if(sig==1) ok=trade.Buy(lot,_Symbol,0.0,sl,tp,"FREQ AGG Buy");
   else       ok=trade.Sell(lot,_Symbol,0.0,sl,tp,"FREQ AGG Sell");
