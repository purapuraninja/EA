//+------------------------------------------------------------------+
//|      XAUUSD_Scalp_Active_Frequent_MT5.mq5                         |
...
// --- Filter 2: Range (looser)
input int    InpMinTrendPoints     = 150;    // smaller => more entries

// --- Filter 3: Impulse (looser)
input double InpImpulseATRMult     = 1.8;    // bigger => more entries (less blocking)
...
input int    InpMaxSpreadPoints    = 800;
...
// (1) HTF trend confirm: EMA fast vs slow in HTF on closed bar
...
// (2) Range filter: require EMA distance >= threshold
...
// (3) Impulse filter: reject if prev candle range > ATR * mult
bool NoImpulse()
{
   double atr[1];
...
bool ApplyStopSafety(bool buy, double entryPrice, double &sl, double &tp)
{
   ...
   // Broker limits (in points)
   int stopsLevel = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
   int freezeLevel= (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_FREEZE_LEVEL);

   // Use the stricter of the two (plus a small buffer)
   int minPts = MathMax(stopsLevel, freezeLevel) + 5;
