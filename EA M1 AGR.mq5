...
// --- Strategy (AGRESIF)
input int    InpEmaFast            = 14;     // lebih sensitif
input int    InpEmaSlow            = 34;
input int    InpRSIPeriod          = 14;
input double InpRSI_BuyMin         = 50.0;   // minimal bias bullish
input double InpRSI_SellMax        = 50.0;   // minimal bias bearish

// Pullback longgar (kejar peluang)
input int    InpPullbackMaxPoints  = 1200;   // sering entry walau pullback dangkal
input int    InpCooldownBars       = 0;      // no delay (agresif)

// --- SL / TP (cepat keluar)
input bool   InpUseATRStops        = true;
input int    InpATRPeriod          = 14;
input double InpATR_SL_Mult        = 1.2;
input double InpATR_TP_Mult        = 1.1;    // TP cepat
input int    InpSL_Points          = 350;
input int    InpTP_Points          = 400;

// --- Compounding (HATI-HATI)
input bool   InpUseCompoundStep    = true;
input double InpBaseBalance        = 100.0;
input double InpBaseLot            = 0.01;
input double InpStepBalance        = 40.0;   // cepat naik lot
input double InpLotStep            = 0.01;
input double InpMinLot             = 0.01;
input double InpMaxLot             = 0.10;   // JANGAN lebih dari ini di M1

// --- Protection / Execution (lebih toleran)
input int    InpMaxSpreadPoints    = 250;    // biar nggak sering ke-block
input int    InpDeviationPoints    = 60;
input bool   InpOnePositionOnly    = true;

// --- Break Even (lebih lambat)
input bool   InpUseBreakEven       = true;
input int    InpBE_StartPoints     = 500;
input int    InpBE_OffsetPoints    = 30;

// --- Trailing (longgar)
input bool   InpUseTrailing        = true;
input int    InpTrailStartPoints   = 700;
input int    InpTrailDistance      = 350;
