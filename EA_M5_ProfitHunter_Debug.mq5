...
//--- DEBUG: relax signal (for troubleshooting 0 trades)
input bool            InpRelaxSignal       = false; // if true: ignore MA direction filter
...
bool SignalD(bool &buy, bool &sell)
{
   ...
   if(InpRelaxSignal)
   {
      // Debug mode: trade MACD cross only (helps confirm EA is alive)
      if(crossUp)   buy=true;
      if(crossDown) sell=true;
   }
   else
   {
      // Normal mode: MACD cross in direction of trend MA
