...
input bool                InpTrailStartUseATR    = true;   // start trailing juga bisa adaptif ATR
input double              InpTrailStart_ATR_Mult = 1.00;   // start trailing ketika profit >= ATR*mult (dalam points)
input int                 InpTrailStartMinPoints = 150;    // floor start trailing (points) agar tidak terlalu cepat
...
//--- BE adaptif ATR (tambahan)
// Jika ON: trigger & offset akan mengikuti ATR agar lebih cocok untuk volatilitas XAU yang berubah-ubah.
// Trigger = max(Trigger_RR/TriggerPoints, ATR*TriggerMult, MinTriggerPoints)
// Offset  = max(OffsetPoints, ATR*OffsetMult, MinOffsetPoints)
input bool                InpBE_UseATRAdaptive   = true;
input double              InpBE_ATR_TriggerMult  = 1.00;   // contoh: 1.0 berarti BE trigger di ~1x ATR (dalam points)
input double              InpBE_ATR_OffsetMult   = 0.10;   // contoh: 0.10 berarti offset SL di ~0.10x ATR
input int                 InpBE_MinTriggerPoints = 120;    // floor trigger (points)
input int                 InpBE_MinOffsetPoints  = 5;      // floor offset (points)
...
void ApplyBreakeven()
{
   ...
   // -----------------------------
   // Trigger BE (dalam POINTS)
   // Prioritas:
   // 1) Trigger RR jika InpBE_Trigger_RR > 0
   // 2) Fallback TriggerPoints
   // 3) Jika InpBE_UseATRAdaptive: max dengan (ATR points * InpBE_ATR_TriggerMult)
   // 4) Floor InpBE_MinTriggerPoints
   // -----------------------------
   double triggerPts = 0.0;

   if(InpBE_Trigger_RR > 0.0 && g_RPoints > 0.0)
      triggerPts = g_RPoints * InpBE_Trigger_RR;
   else if(InpBE_TriggerPoints > 0)
      triggerPts = (double)InpBE_TriggerPoints;

   // ATR adaptif (ambil ATR points sekali saja)
   double atrPts = 0.0;
   if(InpBE_UseATRAdaptive)
   {
      double atr = 0.0;
      if(GetATR(atr) && p > 0.0)
         atrPts = atr / p;
      if(atrPts > 0.0)
         triggerPts = MathMax(triggerPts, atrPts * InpBE_ATR_TriggerMult);
   }

   if(triggerPts < (double)InpBE_MinTriggerPoints)
      triggerPts = (double)InpBE_MinTriggerPoints;

   if(profitPts < triggerPts) return;

   // -----------------------------
   // Offset BE (dalam POINTS)
   // Offset = max(InpBE_OffsetPoints, ATR*OffsetMult, MinOffsetPoints)
   // -----------------------------
   double offsetPts = (double)InpBE_OffsetPoints;
   if(InpBE_UseATRAdaptive && atrPts > 0.0)
      offsetPts = MathMax(offsetPts, atrPts * InpBE_ATR_OffsetMult);

   if(offsetPts < (double)InpBE_MinOffsetPoints)
      offsetPts = (double)InpBE_MinOffsetPoints;

   int digits = Dig();
   double offset = offsetPts * p;
...
void ApplyTrailing()
{
   ...
   // Tambahan: trailing start adaptif ATR
   // Jika ON, maka start trailing minimal akan mengikuti ATR
   // (profit >= ATR * InpTrailStart_ATR_Mult)
   if(InpTrailStartUseATR)
   {
      double atr = 0.0;
      if(GetATR(atr) && p > 0.0)
      {
         double atrPts = atr / p;
         double atrStartPts = atrPts * InpTrailStart_ATR_Mult;

         if(atrStartPts > trailStartPts)
            trailStartPts = atrStartPts;

         // floor agar tidak terlalu cepat
         if(trailStartPts < (double)InpTrailStartMinPoints)
            trailStartPts = (double)InpTrailStartMinPoints;
      }
   }
