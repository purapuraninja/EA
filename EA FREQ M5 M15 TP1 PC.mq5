//+------------------------------------------------------------------+
//|            XAUUSDm_Scalp_Frequent_Final.mq5                       |
//|  MORE FREQUENT ENTRY: HTF confirm + Range + Impulse + StopSafety  |
...
input double InpRSI_BuyMin         = 52.0;   // looser => more entries
input double InpRSI_SellMax        = 48.0;

input int    InpPullbackMaxPoints  = 500;    // wider pullback => more entries
input int    InpCooldownBars       = 2;

// (2) Range Filter (looser)
input int    InpMinTrendPoints     = 120;

// (3) Impulse Filter (looser)
...
input int    InpTP_Points          = 1800;   // if ATR off
...
input int    InpTrailStartPoints   = 1200;
input int    InpTrailDistance      = 800;
...
input double InpTP1_ATR_Mult        = 0.8;   // TP1 = 0.8*ATR
...
// (3) Impulse filter (prev candle range <= ATR*mult)
bool NoImpulse()
{
   double atr[1];
...
{
   // ensure SL not too close to current price (for modify)
   double minDist = MinStopPoints()*_Point;
         ...
         if(!tp1Done && tp1Pts>0 && profitPts>=tp1Pts)
         {
            ...
            if(CanPartialClose(curVol, closeVol))
            {
               ...
               if(trade.PositionClosePartial(ticket, closeVol))
               {
                  ...
                  // move SL to BE after TP1 (optional)
            ...
            {
               // Cannot partial due to min lot; mark done to avoid spam
               GlobalVariableSet(flag, (double)TimeCurrent());
            }
         }
      }
...
int OnInit()
{
   ...
   Print("Frequent EA ready on ",_Symbol," TF=",EnumToString(InpTF)," HTF=",EnumToString(InpHTF));
...
void OnTick()
{
   ...
   bool ok=false;
   if(sig==1) ok=trade.Buy(lot,_Symbol,0.0,sl,tp,"Freq Buy");
   else       ok=trade.Sell(lot,_Symbol,0.0,sl,tp,"Freq Sell");
