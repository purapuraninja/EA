...
int OnInit()
{
      ...
      InpTrailingDistance <= 0.0 || InpTrailingStep < 0.0)
   {
      Print("Parameter exit tidak valid. Pastikan ProfitOffset dan "
            "TrailingDistance > 0 serta Buffer/TrailingStep >= 0.");
      return INIT_PARAMETERS_INCORRECT;
   }

   if(InpUseSoftTrendFilter &&
...
void DumpSymbolSpec()
{
   ...
   if(InpUseRecovery)
   {
      ...
      if(InpRecoveryMinAgeHours > 0.0)
         PrintFormat("  JALUR UMUR: ON. Recovery juga menyala bila basket "
                     "lebih tua dari %.2f jam DAN floating <= -%.2f%% "
                     "ekuitas (kini -%.2f %s). Syaratnya persen, jadi "
                     "tidak perlu disetel ulang saat ukuran akun berubah.",
                     InpRecoveryMinAgeHours, InpRecoveryAgeFloatPct,
                     AccountInfoDouble(ACCOUNT_EQUITY) *
                     InpRecoveryAgeFloatPct / 100.0, acc_ccy);
      else
         Print("  JALUR UMUR: OFF (InpRecoveryMinAgeHours = 0), "
               "hanya jalur floating yang aktif.");
                    ...
                    double be, double price)
{
   ...
   // ---------------- GATE TREN RECOVERY ----------------
   //
   // Ini yang tidak ada saat akun real cent habis pada 12 Agustus 2026.
   //
   // Arah recovery ditetapkan SEKALI saat pemicu (berlawanan dengan basket
   // yang minus) lalu tidak pernah dievaluasi lagi. Grid SELL menaruh
   // limit di ATAS harga, jadi harga naik justru MENGISI ladder-nya.
   // Akibatnya, dari MC-v1.53.xlsx (semua USC):
   //
   //   16:56:21  basket BUY -5.284  | recovery SELL  1 pos  1,37 lot
   //   16:59:33  basket BUY -2.483  | recovery SELL 17 pos 23,29 lot  -4.692
   //   17:07:40  basket BUY    +60  | recovery SELL 21 pos 31,51 lot -17.250
   //   17:22:19  likuidasi          | recovery                       -21.808
   //
   // Harga naik dari 4398,680 ke 4408,547. Basket BUY yang mau diselamatkan
   // SEMBUH SENDIRI sampai untung +60. Recovery-lah yang menghabiskan akun.
   //
   // Sekarang ladder recovery BERHENTI menambah level saat tren melawan
   // arahnya, memakai TrendAllows() yang sama dengan grid utama. Limit yang
   // belum terisi dibatalkan supaya tidak ikut terisi saat harga jalan.
   //
   // Yang TIDAK dilakukan: menutup posisi recovery yang sudah ada. Itu
   // merealisasi kerugian. Posisi yang ada tetap dipegang, trailing dan
   // exit basket terus berjalan, dan begitu tren netral atau berbalik ke
   // arah recovery, ladder lanjut lagi.
   //
   // Gate ini hanya berlaku untuk magic recovery. Grid utama tidak disentuh.
   if(InpRecoveryTrendGate && IsRecoveryMagic(magic) && !TrendAllows(dir))
   {
      int pend_now = CountPendings(magic);
      if(pend_now > 0)
         CancelAllPendings(magic);
      static datetime s_rtrend_log[2] = {0, 0};
      int gi = DirIndex(dir);
      if(TimeCurrent() - s_rtrend_log[gi] >= 60)
      {
         PrintFormat("GATE TREN RECOVERY %s: tren sedang %s, arah recovery "
                     "SALAH. Ladder dihentikan, %d limit dibatalkan. "
                     "%d posisi recovery yang ada tetap dipegang dan exit "
                     "basketnya jalan terus. Ladder lanjut bila tren netral "
                     "atau berbalik.",
                     (dir == POSITION_TYPE_BUY) ? "BUY" : "SELL",
                     (g_trend_state > 0) ? "NAIK" : "TURUN",
                     pend_now, CountPositions(magic));
         s_rtrend_log[gi] = TimeCurrent();
      }
      return;
   }

   int pendings = CountPendings(magic);
   if(pendings >= InpPendingBatch)
      return;
   ...
   while(placed < InpBatchRefill && guard < InpBatchRefill * 5)
   {
...
bool RecoveryAllowed(ENUM_POSITION_TYPE losing_dir)
{
   ...
   //
   // Jalur UMUR (v1.54): basket lebih tua dari ambang DAN floatingnya minus
...
void ManageGrid(ENUM_POSITION_TYPE dir, long magic, bool allow_new_cycle)
{
   ...
   if(nPos == 0)
   {
      ...
      double anchor_new = GlobalVariableGet(GVAnchorName(magic));
      // Ladder awal: InpPendingBatch limit pertama (level 2..).
      // Siklus baru, kedalaman nol, jadi ramp lot penuh dan stride 1.
      // Berhenti begitu broker menolak. Tanpa guard ini, satu kondisi
      // penolakan menghasilkan InpPendingBatch request gagal sekaligus -
      // persis 6 baris invalid price yang muncul di log 6 Aug 08:26:44.
      g_place_reject = false;
      for(int lvl = 2; lvl <= 1 + InpPendingBatch; lvl++)
      {
         if(!PlaceLimitAtLevel(dir, magic, anchor_new, lvl, false) &&
            g_place_reject)
            break;
      }
      return;
      ...
      IsVirtualStopHit(DirSign(dir), price, g_vstop[idx]))
   {
      PrintFormat("EXIT TRIGGER %s magic %I64d: harga %s menyentuh level %s "
                  "| BE %s | %d posisi, %d pending. Menutup basket.",
                  (dir == POSITION_TYPE_BUY) ? "BUY" : "SELL", magic,
                  DoubleToString(price, g_digits),
                  DoubleToString(g_vstop[idx], g_digits),
                  DoubleToString(be, g_digits),
                  nPos, CountPendings(magic));
      g_closing[idx] = true;
      RunClose(dir, magic);
