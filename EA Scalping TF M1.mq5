...
// --- Strategy (active) | M1 tuned
input int    InpEmaFast            = 20;
input int    InpEmaSlow            = 50;
input int    InpRSIPeriod          = 14;
input double InpRSI_BuyMin         = 51.0;   // lebih aktif di M1
input double InpRSI_SellMax        = 49.0;

// Pullback filter (lebih toleran buat noise M1)
input int    InpPullbackMaxPoints  = 700;    // M1: lebih longgar biar sering entry
input int    InpCooldownBars       = 1;      // M1: jangan kelamaan nahan entry

// --- SL / TP (M1 scalping)
input bool   InpUseATRStops        = true;
input int    InpATRPeriod          = 14;
input double InpATR_SL_Mult        = 1.0;    // M1: lebih ketat
input double InpATR_TP_Mult        = 1.3;    // M1: TP lebih realistis
input int    InpSL_Points          = 300;    // fallback kalau ATR off
input int    InpTP_Points          = 450;

// --- Compounding (step) | aman utk balance kecil
...
input double InpMaxLot             = 0.10;   // M1: turunin biar gak kebablasan

// --- Protection / Execution (M1 spread spike)
input int    InpMaxSpreadPoints    = 180;    // M1 Exness XAU: biar gak terlalu sering ke-block
input int    InpDeviationPoints    = 50;
input bool   InpOnePositionOnly    = true;

// --- Break Even (M1)
input bool   InpUseBreakEven       = true;
input int    InpBE_StartPoints     = 400;    // lebih realistis utk M1
input int    InpBE_OffsetPoints    = 80;

// --- Trailing (M1)
input bool   InpUseTrailing        = true;
input int    InpTrailStartPoints   = 600;
input int    InpTrailDistance      = 250;
