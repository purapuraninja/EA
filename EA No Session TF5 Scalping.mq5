...
input double InpRSI_BuyMin         = 53.0;
input double InpRSI_SellMax        = 47.0;

// Pullback filter: enter only when price is close to EMA fast (on closed bar)
input int    InpPullbackMaxPoints  = 400;   // max distance from EMA fast to allow entry
input int    InpCooldownBars       = 3;     // wait N bars after a trade before allowing new entry
...
input int    InpTP_Points          = 2000;  // used if ATR off
...
input int    InpMaxSpreadPoints    = 500;   // allow high spread for testing; tune later
