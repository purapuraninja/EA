...
//--- v1.55-Gilingan: state rantai recovery (cap 10 + bailout-reset) ----
// g_chain_loss mengakumulasi rugi rantai per arah [0=BUY,1=SELL].
// g_anchor_lot_override dipakai OpenAnchor sekali untuk lot recovery.
double   g_chain_loss[2]        = {0.0, 0.0};
int      g_bailout_count[2]     = {0, 0};
bool     g_bailing_out[2]       = {false, false};
double   g_anchor_lot_override[2] = {0.0, 0.0};
...
bool OpenAnchor(ENUM_POSITION_TYPE dir, long magic)
{
   ...
   int dirIdx0 = (dir == POSITION_TYPE_BUY) ? 0 : 1;
   if(g_anchor_lot_override[dirIdx0] > 0.0)
   {
      lot = NormLot(g_anchor_lot_override[dirIdx0]);
      g_anchor_lot_override[dirIdx0] = 0.0;   // sekali pakai
   }
   bool ok;
   double price;
   if(dir == POSITION_TYPE_BUY)
...
//+------------------------------------------------------------------+
//| v1.55-Gilingan: hitung lot recovery dari rugi rantai (bukan 1.65) |
//|    lot = (|rugi_rantai| + target_profit) / (TP x 100)             |
//|    TP recovery = ProfitOffset x InpGilinganTpMult (default 15)    |
//+------------------------------------------------------------------+
double GilinganRecoveryLot(int dirIdx)
{
   double chain_loss = MathAbs(g_chain_loss[dirIdx]);
   if(chain_loss <= 0.0)
      return InpBaseLot;

   double recov_tp     = InpProfitOffset * 15.0;      // ~ $4.5 target lebar
   double target_profit = chain_loss * 0.20;           // 20% margin di atas rugi
   double lot = (chain_loss + target_profit) / (recov_tp * 100.0);

   lot = NormLot(lot);
   double min_lot = InpBaseLot * 2.0;
   double max_lot = InpBaseLot * 20.0;
...
//+------------------------------------------------------------------+
//| v1.55-Gilingan: reset rantai setelah basket menang (TP sukses)    |
//+------------------------------------------------------------------+
void GilinganReset(int dirIdx)
{
   if(g_chain_loss[dirIdx] != 0.0 || g_bailout_count[dirIdx] > 0)
   {
      PrintFormat("GILINGAN RESET %s: rantai %.2f pulih dalam %d bailout.",
                  (dirIdx == 0) ? "BUY" : "SELL",
                  g_chain_loss[dirIdx], g_bailout_count[dirIdx]);
   }
   g_chain_loss[dirIdx]    = 0.0;
   g_bailout_count[dirIdx] = 0;
   g_bailing_out[dirIdx]   = false;
}

//+------------------------------------------------------------------+
//| v1.55-Gilingan: exit atomik — pasang SL SERAGAM di target profit  |
//| pada SEMUA posisi basket. Eksekusi server-side (tutup serentak).  |
//| Dipasang HANYA saat harga sudah melewati target (SL valid).       |
//| Mengembalikan true bila berhasil memasang >= 1 SL.                |
//+------------------------------------------------------------------+
bool GilinganAtomicSLExit(ENUM_POSITION_TYPE dir, long magic, double be, double price)
{
   ...
   if(dir == POSITION_TYPE_BUY)
   {
      target = be + ProfitOffsetFor(magic);
      if(target >= price - eff_min) return false;   // belum lewat target
   }
   else
   {
      target = be - ProfitOffsetFor(magic);
      if(target <= price + eff_min) return false;
   }
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      double cur_sl = PositionGetDouble(POSITION_SL);
      if(MathAbs(cur_sl - target) < g_tick_size * 0.5) continue;   // sudah pas

      if(trade.PositionModify(tk, target, 0.0))
         armed++;
   }

   if(armed > 0)
   {
      PrintFormat("GILINGAN ATOMIC SL %s: %d/%d posisi dipasang SL seragam di %s "
                  "(target profit). Tutup server-side saat tersentuh.",
                  BasketName(magic), armed, nPos, DoubleToString(target, g_digits));
   }
   return (armed > 0);
}
...
void ManageGrid(ENUM_POSITION_TYPE dir, long magic, bool allow_new_cycle)
{
   ...
   if(nPos == 0)
   {
      ...
      // v1.55-Gilingan: kalau rantai masih rugi, pakai lot recovery.
      {
         int dirIdx0 = (dir == POSITION_TYPE_BUY) ? 0 : 1;
         if(g_chain_loss[dirIdx0] < 0.0)
            g_anchor_lot_override[dirIdx0] = GilinganRecoveryLot(dirIdx0);
         else
            g_anchor_lot_override[dirIdx0] = 0.0;
      }
   ...
   //--- v1.55-Gilingan: CAP 10 LEVEL + BAILOUT-RESET ---
   // Kalau basket sudah penuh (>= 10 posisi), tutup semua, booking rugi,
   // lalu buka ulang searah dengan lot recovery pada siklus berikutnya.
   {
      int dirIdx0 = (dir == POSITION_TYPE_BUY) ? 0 : 1;
      if(nPos >= 10)
      {
         if(!g_bailing_out[dirIdx0])
         {
            g_bailing_out[dirIdx0] = true;
            double basket_pl  = BasketFloating(magic);
            double basket_vol = BasketVolume(magic);
            CancelAllPendings(magic);
            CloseBasket(magic);
            g_chain_loss[dirIdx0] += basket_pl;
            g_bailout_count[dirIdx0]++;
            PrintFormat("GILINGAN BAILOUT %s: %d pos / %.2f lot ditutup, "
                        "rugi basket %.2f, rugi rantai %.2f (bailout #%d). "
                        "Buka ulang searah dengan lot recovery.",
                        BasketName(magic), nPos, basket_vol, basket_pl,
                        g_chain_loss[dirIdx0], g_bailout_count[dirIdx0]);
         }
         return;   // tunggu close selesai; siklus baru mulai tick berikutnya
      }
   }
   ...
   //--- v1.55-Gilingan: EXIT ATOMIK SL-IN-PROFIT ---
   // Coba pasang SL seragam di target profit dulu. Kalau berhasil,
   // biarkan server yang menutup serentak — tidak perlu ladder/jaring.
   if(InpUseVirtualTrailing && GilinganAtomicSLExit(dir, magic, be, price))
   {
      // Reset rantai bila ini menutup rantai recovery yang sedang berjalan.
      int dirIdx0 = (dir == POSITION_TYPE_BUY) ? 0 : 1;
      if(g_chain_loss[dirIdx0] < 0.0)
      {
         // Basket yang menang (target profit tercapai) memulihkan rantai.
         GilinganReset(dirIdx0);
      }
      return;   // SL terpasang; eksekusi diserahkan server
   }
