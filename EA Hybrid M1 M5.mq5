//+------------------------------------------------------------------+
//|   XAUUSDm_Hybrid_M1Entry_M15Filter_Full.mq5                       |
//|   HYBRID: M1 Entry (active) + M15 Trend Filter (safer)            |
//|   Features: ATR/FIX SLTP, Compounding, BE, Trailing, Partial TP1  |
//|   Safety: StopLevel/FrozenLevel adjust                             |
//+------------------------------------------------------------------+
#property strict
...
//====================== INPUT ======================================
input int             InpMagic              = 2026012402;
input ENUM_TIMEFRAMES InpTF_Entry           = PERIOD_M1;    // Entry TF
input ENUM_TIMEFRAMES InpTF_Filter          = PERIOD_M15;   // Trend Filter TF

// --- Entry indicators (M1)
input int    InpEmaFast            = 20;
input int    InpEmaSlow            = 50;
input int    InpRSIPeriod          = 14;
input double InpRSI_BuyMin         = 50.0;     // hybrid: not too strict
input double InpRSI_SellMax        = 50.0;

// --- Entry behavior (M1)
input int    InpPullbackMaxPoints  = 3500;     // gold can run fast
input int    InpMinTrendPoints     = 20;       // small EMA separation allowed
input double InpImpulseATRMult     = 8.0;      // tolerate spikes more (M1 noisy)
input int    InpCooldownBars       = 1;        // small cooldown

// --- SL/TP
input bool   InpUseATRStops        = true;
input int    InpATRPeriod          = 14;
input double InpATR_SL_Mult        = 1.2;
input double InpATR_TP_Mult        = 1.4;

input int    InpSL_Points          = 900;      // if ATR off (M1)
input int    InpTP_Points          = 1200;     // if ATR off (M1)
...
input int    InpMaxSpreadPoints    = 500;      // M1 needs more tolerance; spread kamu ~160 OK
input int    InpDeviationPoints    = 80;
input bool   InpOnePositionOnly    = true;

// --- Break-even
input bool   InpUseBreakEven       = true;
input int    InpBE_StartPoints     = 500;
input int    InpBE_OffsetPoints    = 40;

// --- Trailing
input bool   InpUseTrailing        = true;
input int    InpTrailStartPoints   = 700;
input int    InpTrailDistance      = 550;
...
input double InpTP1_ATR_Mult        = 0.6;     // TP1 closer for M1
input int    InpTP1_Points          = 450;     // if ATR off
input bool   InpTP1_MoveSLToBE      = true;
input int    InpTP1_BE_OffsetPoints = 20;
...
bool IsNewBar()
{
   datetime t = iTime(_Symbol, InpTF_Entry, 0);
...
//====================== FILTER (M15 trend confirm) ==================
bool HTFTrendOK(bool buy)
{
   double f[1], s[1];
   // use closed M15 bar (shift=1)
...
bool NoImpulse()
{
   ...
   double hi = iHigh(_Symbol, InpTF_Entry, 1);
   double lo = iLow(_Symbol,  InpTF_Entry, 1);
   if(hi<=0 || lo<=0 || atr[0]<=0) return false;
...
int GetSignal()
{
   ...
   // Pullback distance to EMA fast (but wide tolerance)
   double distPts = MathAbs(close1 - f[0])/_Point;
   bool pullbackOK = (distPts <= InpPullbackMaxPoints);

   // Range + impulse filters (M1)
   if(!NotRanging(f[0], s[0])) return 0;
   if(!NoImpulse())            return 0;

   // M15 trend filter
   bool htfBuyOk  = HTFTrendOK(true);
   bool htfSellOk = HTFTrendOK(false);

   if(upTrend && htfBuyOk  && pullbackOK && r[0] >= InpRSI_BuyMin)  return 1;
   if(downTrend && htfSellOk && pullbackOK && r[0] <= InpRSI_SellMax) return -1;
...
int OnInit()
{
   ...
   // Entry TF handles
   hFast = iMA(_Symbol, InpTF_Entry, InpEmaFast, 0, MODE_EMA, PRICE_CLOSE);
   hSlow = iMA(_Symbol, InpTF_Entry, InpEmaSlow, 0, MODE_EMA, PRICE_CLOSE);
   hRSI  = iRSI(_Symbol, InpTF_Entry, InpRSIPeriod, PRICE_CLOSE);
   hATR  = iATR(_Symbol, InpTF_Entry, InpATRPeriod);

   // HTF filter handles
   hFastHTF = iMA(_Symbol, InpTF_Filter, InpEmaFast, 0, MODE_EMA, PRICE_CLOSE);
   hSlowHTF = iMA(_Symbol, InpTF_Filter, InpEmaSlow, 0, MODE_EMA, PRICE_CLOSE);
   ...
   lastBarTime = iTime(_Symbol, InpTF_Entry, 0);
   cooldownLeft = 0;

   Print("HYBRID M1->M15 ready on ", _Symbol,
         " EntryTF=", EnumToString(InpTF_Entry),
         " FilterTF=", EnumToString(InpTF_Filter),
...
void OnTick()
{
   ...
   // entry only on new M1 bar (prevents spam)
   if(!IsNewBar()) return;
   ...
   bool ok=false;
   if(sig==1) ok = trade.Buy(lot, _Symbol, 0.0, sl, tp, "HYB Buy");
   else       ok = trade.Sell(lot, _Symbol, 0.0, sl, tp, "HYB Sell");
