//+------------------------------------------------------------------+
//|                                              EA_Grid.mq5    |
//|  EA grid counter-trend (akun cent, mode hedge).           |
//|                                                                  |
...
//|                                                                  |
//|  === v1.54: RUIN GATE + THROTTLE INVENTARIS ===                  |
//|  Tiga akun (MC-v1.53-C, MC-v1.53-D, MC-v1.53-FTG) mati oleh       |
//|  STOP-OUT broker, bukan oleh keputusan EA: deal-deal terakhirnya |
//|  bertanda [so ...] - ditutup server satu per satu dari posisi     |
//|  rugi TERBESAR (hasil ramp lot) sampai balance negatif. Tanpa     |
//|  cut loss, akun tidak pernah mati karena loss terealisasi;        |
//|  matinya karena margin. Satu-satunya variabel yang bisa          |
//|  dikontrol adalah KECEPATAN DAN VOLUME tangga mengisi posisi:     |
//|                                                                  |
//|  1. Ruin gate: refill ladder berhenti proporsional terhadap       |
//|     jarak ruin basket (equity / biaya per 1 USD gerak). Zona      |
//|     genting membatasi pending, zona kritis mematikan ladder.      |
//|     TIDAK ADA POSISI YANG DITUTUP oleh gate ini.                  |
//|  2. PROBE tidak lagi memakai ramp lot: satu limit LOT DASAR,      |
//|     dan cooldown probe naik setiap kali burst terulang - pola     |
//|     7 Sep (3,42 lot terisi dalam 5 menit) lahir dari siklus       |
//|     rilis-terlalu-cepat lalu refill batch penuh.                  |
//|  3. Eksekusi exit DITUNDA selama gate BLOCKED (burst) dan saat    |
//|     spread melebar, maksimum InpBurstCloseDelaySec. Data 7 Sep    |
//|     11:40: basket ditutup tepat di harga terendah hari itu.       |
//|     Ratchet tetap berjalan; verifikasi margin tetap menagih.      |
//|  4. Edge exit kini juga proporsional LOT basket (InpEdgePerLot):  |
//|     drift penutupan menumpuk per lot, bukan per posisi.           |
...
// v1.54: DEFAULT OFF. Data MC-v1.53-D (24-25 Agt, akun yang hangus):
// 149 posisi RECOV (9.20 lot, 0.05-0.15) merealisasikan -3.241,55 -
// rata-rata -21,76 per posisi, LEBIH BURUK dari basket utama yang
// dilayaninya (-2,49 per posisi) - dan ikut ter-stop-out bersama
// akunnya. Tangga recovery ikut terisi di pasar yang jatuh, bukan
// menyelamatkan. Mekanismenya tetap tersedia untuk pengujian ulang.
input bool   InpUseRecovery         = false;  // Aktifkan recovery grid (v1.54: default OFF)
...
// v1.54: default ON (600 points = 0.60 USD pada simbol 3 digit seperti
// XAUUSDm; spread normal terukur 0.24). Blokir anchor + refill saat
// spread melebar (news), bukan hanya saat close. 0 = off seperti
// sebelumnya. Kalibrasi ulang per simbol dari log OnInit.
input double InpMaxSpread      = 600;     // Spread maksimum (points, 0=off). XAUUSDc 3-digit: spread normal 150-350
...
input group  "=== Ruin Gate & Eksekusi (v1.54) ==="
// Tanpa cut loss, akun tidak mati karena loss terealisasi - matinya
// karena MARGIN (bukti: deal terakhir ketiga akun yang hangus bertanda
// [so ...] = stop out broker). Jadi yang diatur bukan kapan keluar,
// tapi kapan boleh MENAMBAH eksposur. Jarak ruin = berapa USD harga
// masih boleh berjalan MELAWAN basket sebelum equity habis. Hitungan
// ini menuju equity NOL, sedangkan stop-out broker terjadi LEBIH AWAL;
// perlakukan angkanya sebagai batas atas dan kalibrasi per akun.
input bool   InpUseRuinGate        = true;   // Ruin gate: batasi refill saat jarak ruin tipis
input double InpMinRuinDistance    = 25.0;   // Jarak ruin "genting": pending dibatasi (USD)
input double InpRuinDistanceTight  = 10.0;   // Jarak ruin "kritis": ladder mati (USD)
input int    InpRuinPendingMax     = 3;      // Pending maksimum saat genting (level)
input bool   InpProbeSingleLimit   = true;   // PROBE: satu limit LOT DASAR (tanpa ramp)
input int    InpProbeCooldownGrowSec = 30;   // Cooldown probe bertambah ini per burst berulang (0=off)
input int    InpProbeCooldownMaxSec  = 90;   // Batas atas cooldown probe (0=off)
input bool   InpUseBurstCloseDelay = true;   // Tunda eksekusi exit selama gate BLOCKED
input int    InpBurstCloseDelaySec = 45;     // Lama maksimum penundaan exit saat burst (detik)
input double InpEdgePerLot         = 0.10;   // Tambahan margin exit per LOT basket (USD, 0=off)
input double InpCloseSpreadMax     = 1.50;   // Spread maksimum utk MEMULAI close basket (USD, 0=off)

input group  "=== News Filter (siklus baru) ==="
input bool   InpUseNewsFilter       = false;   // Default OFF
...
input bool   InpUseFloatingProtection = true;  // Aktif/nonaktif proteksi floating
...
void UiLoadState()
{
   ...
   // v1.54: kunci GV BARU. Kunci lama ("recov") sengaja diabaikan: default
   // input kini OFF, sedangkan nilai ON yang tertanam di GV lama akan
   // diam-diam menyalakan kembali modul yang sudah diputuskan dimatikan.
   g_ui_recov  = UiLoadBool("recov_v154", InpUseRecovery);
...
//--- v1.54: state tambahan --------------------------------------------
// Berapa kali arah ini jatuh KEMBALI ke BLOCKED dari PROBE. Dipakai
// menaikkan cooldown probe: pola 7 Sep (fill 3,42 lot dalam 5 menit)
// lahir dari siklus rilis-terlalu-cepat - terisi, hening 45 detik,
// rilis, refill batch, terisi lagi. Cooldown statis membiarkan siklus
// itu berulang; cooldown yang naik per kegagalan memutusnya.
int      g_fast_probe_streak[2]            = {0, 0};
// Waktu vstop PERTAMA kali tersentuh selama gate masih BLOCKED. Dasar
// batas waktu penundaan eksekusi exit di tengah burst (v1.54).
datetime g_burst_close_since[BASKET_SLOTS] = {0, 0, 0, 0};
...
int OnInit()
{
   ...
   // v1.54: ruin gate. Dua ambangnya bertingkat: kritis harus lebih ketat
   // (lebih kecil) dari genting, dan pending maksimum saat genting minimal
   // satu level supaya BE tetap punya mekanisme perbaikan.
   if(InpUseRuinGate &&
      (InpMinRuinDistance <= 0.0 || InpRuinDistanceTight <= 0.0 ||
       InpRuinDistanceTight > InpMinRuinDistance || InpRuinPendingMax < 1))
   {
      Print("Parameter ruin gate tidak valid: wajib 0 < RuinDistanceTight "
            "<= MinRuinDistance dan RuinPendingMax >= 1.");
      return INIT_PARAMETERS_INCORRECT;
   }

   if(InpEdgePerLot < 0.0 || InpCloseSpreadMax < 0.0 ||
      InpBurstCloseDelaySec < 0 || InpProbeCooldownGrowSec < 0 ||
      InpProbeCooldownMaxSec < 0)
   {
      Print("Parameter v1.54 tidak valid: EdgePerLot, CloseSpreadMax, "
            "delay burst, dan cooldown probe tidak boleh negatif.");
      return INIT_PARAMETERS_INCORRECT;
   }

   if(InpProbeCooldownMaxSec > 0 &&
      InpProbeCooldownMaxSec < InpTrendProbeCooldownSec)
   {
      Print("InpProbeCooldownMaxSec tidak boleh lebih kecil dari "
            "InpTrendProbeCooldownSec (batas atas di bawah nilai dasar).");
      return INIT_PARAMETERS_INCORRECT;
   }
...
void DumpSymbolSpec()
{
   ...
   // v1.54: konfigurasi ruin gate dan eksekusi baru, supaya satu log
   // OnInit cukup untuk memeriksa ambang yang benar-benar berlaku.
   if(InpUseRuinGate)
      PrintFormat("Ruin gate : ON, ladder dibatasi saat jarak ruin basket "
                  "<= %.1f USD (pending max %d, refill 1/tick), MATI saat "
                  "<= %.1f USD. Tidak menutup posisi apa pun.",
                  InpMinRuinDistance, InpRuinPendingMax,
                  InpRuinDistanceTight);
   else
      Print("Ruin gate : OFF");
   PrintFormat("v1.54    : probe %s, cooldown %d+%ds per burst berulang "
               "(max %ds) | burst close delay %s (%ds) | edge/lot %.2f USD "
               "| close spread max %s USD",
               InpProbeSingleLimit ? "1 limit LOT DASAR" : "ramp penuh",
               InpTrendProbeCooldownSec, InpProbeCooldownGrowSec,
               InpProbeCooldownMaxSec,
               InpUseBurstCloseDelay ? "ON" : "OFF", InpBurstCloseDelaySec,
               InpEdgePerLot,
               (InpCloseSpreadMax > 0.0)
                  ? DoubleToString(InpCloseSpreadMax, 2) : "OFF");
                  ...
                  const double &dparam, const string &sparam)
{
   ...
   {
      g_ui_recov = !g_ui_recov; UiSaveBool("recov_v154", g_ui_recov);
...
bool FastGateProbeReady(ENUM_POSITION_TYPE dir, long magic)
{
   ...
   return (TimeCurrent() - g_fast_probe_place[di] >=
           FastGateProbeCooldownSec(di));
}

// v1.54: cooldown probe DINAMIS. Basis InpTrendProbeCooldownSec, naik
// InpProbeCooldownGrowSec untuk setiap kali arah ini jatuh kembali ke
// BLOCKED dari PROBE (streak), dibatasi InpProbeCooldownMaxSec. Cooldown
// statis membiarkan siklus fill-hening-refill mengulang diri dengan jeda
// yang sama persis walau bola tanggung selalu berulang.
int FastGateProbeCooldownSec(int di)
{
   int cd = InpTrendProbeCooldownSec;
   if(InpProbeCooldownGrowSec > 0 && g_fast_probe_streak[di] > 0)
      cd += InpProbeCooldownGrowSec * g_fast_probe_streak[di];
   if(InpProbeCooldownMaxSec > 0 && cd > InpProbeCooldownMaxSec)
      cd = InpProbeCooldownMaxSec;
   return cd;
}

// v1.54: apakah basket arah ini sedang dalam burst (gate BLOCKED)?
// Dipakai untuk menunda eksekusi exit sampai harga tenang - menutup
// 5-7 lot di puncak spike berarti menerima fill terburuk hari itu.
bool FastGateBurstNow(ENUM_POSITION_TYPE dir, long magic)
{
   return (InpUseFastTrendGate && FastGateApplies(magic) &&
           g_fast_gate_state[DirIndex(dir)] == TREND_GATE_BLOCKED);
}
...
{
   int di = DirIndex(dir);
   int  prev    = g_fast_gate_state[di];
   bool changed = (prev != TREND_GATE_BLOCKED);
   ...
   if(changed)
   {
      ...
      // v1.54: jatuh ke BLOCKED DARI PROBE berarti rilis sebelumnya
      // terlalu dini - burst menyala lagi. Streak menaikkan cooldown
      // probe berikutnya, jadi tiap kegagalan menahan release makin lama.
      if(prev == TREND_GATE_PROBE && InpProbeCooldownGrowSec > 0)
      {
         g_fast_probe_streak[di]++;
         PrintFormat("FAST TREND GATE %s: PROBE gagal menahan burst "
                     "(streak %d). Cooldown probe berikutnya %d detik.",
                     (dir == POSITION_TYPE_BUY) ? "BUY" : "SELL",
                     g_fast_probe_streak[di], FastGateProbeCooldownSec(di));
      }
                             ...
                             string reason, datetime now, double mid)
{
   ...
   if(state == TREND_GATE_BLOCKED)
   {
      ...
      g_fast_gate_state[di] = TREND_GATE_PROBE;
      g_fast_probe_clean[di] = false;
      int probe_cd = FastGateProbeCooldownSec(di);
      PrintFormat("FAST TREND GATE %s -> PROBE: tenang %d detik, retrace %.2f "
                  "(butuh %.2f). Ladder lama boleh lanjut satu order per %d "
                  "detik (streak %d); anchor baru tetap ditahan.",
                  (dir == POSITION_TYPE_BUY) ? "BUY" : "SELL",
                  (int)quiet_for, retrace, need, probe_cd,
                  g_fast_probe_streak[di]);
      return;
   }
   ...
   if(state == TREND_GATE_PROBE && quiet_for >= InpTrendNormalSec)
   {
      ...
      g_fast_probe_clean[di] = true;
      // v1.54: cukup lama benar-benar tenang - siklus hukuman cooldown
      // selesai, probe kembali memakai cooldown dasar.
      g_fast_probe_streak[di] = 0;
...
string FastGateStatusText()
{
   ...
   return StringFormat("BUY %s | SELL %s | live %.2fATR ER%.2f | fill %d/%d | "
      ...
      FastGateProbeCooldownSec(DirIndex(POSITION_TYPE_BUY)),
      g_fast_probe_streak[0], g_fast_probe_streak[1]);
}
...
//+------------------------------------------------------------------+
//| v1.54: volume total pending grid - eksposur yang akan lahir bila  |
//| seluruhnya terisi. Dipakai pelaporan ruin gate.                   |
//+------------------------------------------------------------------+
double PendingVolume(long magic)
{
   double v = 0.0;
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
                       ...
                       int level, bool freeze_ramp)
{
   ...
   // v1.54: PROBE memakai LOT DASAR. Satu limit probe dengan ramp lot
   // (0.05-0.15 di level dalam) adalah kebocoran yang sama dengan batch
   // penuh: satu placement tetap menaruh eksposur besar di depan harga,
   // dan burst berikutnya mengisinya dalam satu tick.
   if(InpProbeSingleLimit && FastGateApplies(magic) &&
      g_fast_gate_state[DirIndex(dir)] == TREND_GATE_PROBE)
      freeze_ramp = true;
                    ...
                    double be, double price)
{
   ...
   // ---------------- RUIN GATE (v1.54) ----------------
   // Sisi "mengecek margin sebelum refill". Fast gate hanya melihat
   // PERILAKU HARGA; gate ini melihat KONDISI AKUN: seberapa jauh harga
   // masih boleh berjalan melawan basket sebelum equity habis. Ketiga
   // akun yang hangus mati oleh stop-out pada jarak ruin 3-5 USD, dengan
   // ladder yang TERUS MENGISI sampai detik terakhir. Gate ini memutus
   // itu: zona kritis mematikan ladder dan membatalkan pending, zona
   // genting membatasi pending dan laju refill. TIDAK ADA POSISI YANG
   // DITUTUP - hanya bahan bakar yang dikurangi.
   int rzone = RuinZoneForBasket(magic);
   int rslot = SlotOf(magic);
   if(rzone != g_ruin_zone_last[rslot])
   {
      static datetime s_ruin_zone_log[BASKET_SLOTS] = {0, 0, 0, 0};
      if(TimeCurrent() - s_ruin_zone_log[rslot] >= 30)
      {
         double ruin_now = RuinDistance(BasketVolume(magic));
         string zname = (rzone == 0) ? "KRITIS (ladder mati)"
                      : (rzone == 1) ? "GENTING (pending dibatasi)"
                                     : "AMAN (ladder normal)";
         PrintFormat("RUIN GATE %s magic %I64d -> %s: jarak ruin %.1f USD "
                     "(ambang genting %.1f, kritis %.1f). Basket %d posisi "
                     "%.2f lot + %.2f lot pending tetap dipegang, exit tetap "
                     "jalan. TANPA CUT LOSS.",
                     BasketName(magic), magic, zname,
                     (ruin_now >= DBL_MAX / 2.0) ? 9999.0 : ruin_now,
                     InpMinRuinDistance, InpRuinDistanceTight,
                     CountPositions(magic), BasketVolume(magic),
                     PendingVolume(magic));
         s_ruin_zone_log[rslot] = TimeCurrent();
      }
      g_ruin_zone_last[rslot] = rzone;
   }
   ...
   // Genting: pending dibatasi walau fast gate sedang longgar. Trim di
   // sini memanfaatkan jalur trim yang sudah ada di bawah (pendings >
   // target -> cancel), jadi tidak ada jalur cancel kedua yang baru.
   if(rzone == 1)
      pending_target = MathMin(pending_target, InpRuinPendingMax);
   ...
   // v1.54: zona genting juga membatasi LAJU refill ke satu level per
   // tick, walau fast gate mengizinkan batch. Dua mekanisme burst fill
   // (batch 6 sekaligus + banyak tick beruntun) jadi tertutup dua lapis.
   if(rzone == 1)
      refill_count = MathMin(refill_count, 1);
   // v1.54: spread lebar menahan REFILL juga, bukan hanya anchor. Pasang
   // limit saat spread melebar berarti menempatkan level di dalam spread
   // yang segitu, sehingga fill berikutnya lahir di harga buruk.
   if(!SpreadOK())
      return;
...
void ClearCycleState(ENUM_POSITION_TYPE dir, long magic)
{
   ...
   // v1.54: state siklus tambahan ikut dibersihkan. Timer burst close
   // harus nol supaya basket berikutnya tidak mewarisi sisa hitungan
   // mundur, dan zona ruin mulai lagi dari asumsi paling longgar.
   g_burst_close_since[idx] = 0;
   g_ruin_zone_last[idx]    = 2;
...
//+------------------------------------------------------------------+
//| v1.54: zona ruin basket arah ini, dihitung dari volume basket     |
//| yang SUDAH terisi (pending sengaja TIDAK dihitung supaya zonanya  |
//| stabil - kalau ikut dihitung, cancel mengurangi eksposur fiktif,  |
//| zona membaik, refill jalan lagi, dan gate osilasi tanpa henti).   |
//|                                                                   |
//|   2 = aman    : ruin > MinRuinDistance     -> ladder normal       |
//|   1 = genting : ruin <= MinRuinDistance    -> pending dibatasi    |
//|   0 = kritis  : ruin <= RuinDistanceTight  -> ladder mati         |
//|                                                                   |
//| Zona ini TIDAK PERNAH menutup posisi. Yang dikontrol hanya        |
//| berapa banyak bahan bakar yang masih boleh dipajang di depan      |
//| harga. Basket yang ada tetap dipegang dan exit tetap berjalan.    |
//+------------------------------------------------------------------+
int RuinZoneForBasket(long magic)
{
   if(!InpUseRuinGate) return 2;
   double ruin = RuinDistance(BasketVolume(magic));
   if(ruin >= DBL_MAX / 2.0) return 2;
   if(ruin <= InpRuinDistanceTight) return 0;
   if(ruin <= InpMinRuinDistance)   return 1;
   return 2;
}
...
void ManageGrid(ENUM_POSITION_TYPE dir, long magic, bool allow_new_cycle)
{
      ...
      IsVirtualStopHit(DirSign(dir), price, g_vstop[idx]))
   {
      ...
      // v1.54: drift penutupan menumpuk per LOT, bukan per posisi -
      // menutup 7.58 lot campuran 0.01-0.15 makan jauh lebih lama daripada
      // 61 posisi 0.01 dengan total volume yang jauh lebih kecil. Edge
      // kini menjumlahkan komponen per posisi DAN per lot (0 = off).
      ...
      if(edge < need - g_tick_size * 0.5)
      {
         if(TimeCurrent() - g_exit_block_log[idx] >= 30)
         {
            PrintFormat("EXIT DITAHAN %s magic %I64d: level %s kena, tapi "
                        ...
                        "%.2f + %d posisi x %.3f + %.2f lot x %.3f). "
                        "Penutupan butuh waktu dan harga masih bisa lari. "
                        "Basket jalan terus.",
                        (dir == POSITION_TYPE_BUY) ? "BUY" : "SELL", magic,
                        DoubleToString(g_vstop[idx], g_digits),
                        DoubleToString(price, g_digits),
      ...
      {
         // v1.54: JANGAN MENEMBAK DI TENGAH SPIKE. Data 7 Sep 11:40:
         // basket ditutup tepat di harga terendah hari itu (avg exit
         // 4387.7; jam 11:46-11:48 harga sudah 4388-4391). Menutup 5-7
         // lot saat gate BLOCKED berarti menerima fill terburuk hari itu.
         // Selama burst, eksekusi ditunda maksimum InpBurstCloseDelaySec:
         // ratchet tetap berjalan dan tidak mundur, dan kalau harga mundur
         // ke balik level, timer ter-reset - exit dicoba lagi pada
         // penyentuhan berikutnya. Verifikasi margin di atas tetap syarat
         // mutlak; penundaan hanya memindahkan WAKTU eksekusi.
         bool burst = FastGateBurstNow(dir, magic);
         if(burst && g_burst_close_since[idx] == 0)
            g_burst_close_since[idx] = TimeCurrent();

         double spr_now = ask - bid;
         if(InpCloseSpreadMax > 0.0 && spr_now > InpCloseSpreadMax)
         {
            if(TimeCurrent() - g_exit_block_log[idx] >= 30)
            {
               PrintFormat("EXIT DITUNDA %s magic %I64d: spread %.2f USD > "
                           "batas close %.2f USD. Level %s tetap terpasang, "
                           "eksekusi dicoba lagi tick berikutnya.",
                           (dir == POSITION_TYPE_BUY) ? "BUY" : "SELL", magic,
                           spr_now, InpCloseSpreadMax,
                           DoubleToString(g_vstop[idx], g_digits));
               g_exit_block_log[idx] = TimeCurrent();
            }
            return;   // tidak ada ladder / jaring pada tick tertunda
         }

         if(burst && InpUseBurstCloseDelay &&
            TimeCurrent() - g_burst_close_since[idx] < InpBurstCloseDelaySec)
         {
            if(TimeCurrent() - g_exit_block_log[idx] >= 30)
            {
               PrintFormat("EXIT DITUNDA (BURST) %s magic %I64d: gate BLOCKED "
                           "(%s). Eksekusi ditunda maksimum %d detik supaya "
                           "menutup di harga tenang, bukan di puncak spike.",
                           (dir == POSITION_TYPE_BUY) ? "BUY" : "SELL", magic,
                           g_fast_gate_reason[DirIndex(dir)],
                           InpBurstCloseDelaySec);
               g_exit_block_log[idx] = TimeCurrent();
            }
            return;   // tidak ada ladder / jaring pada tick tertunda
         }
      ...
      g_burst_close_since[idx] = 0;   // level tidak tersentuh tick ini
