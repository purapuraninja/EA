...
#property description "EA HybridTP2 v3.02 (MaxATR/MaxSL filter, trail tuning)"
...
input string InpEAName="EA_New_Multy_Fixed_HybridTP2_v302";
...
input int InpMaxSLPoints=0; // 0=off; jika >0 maka ENTRY di-skip bila SLPoints melebihi nilai ini
// fallback fixed SL points
input int                 InpATRPeriod           = 14;
input double              InpSL_ATR_Mult         = 1.25;

input ENUM_TP_MODE        InpTPMode              = TP_RR_MULT;
input int                 InpTPPoints            = 420;
input double              InpTP_RR               = 4.0;
input double              InpTP_ATR_Mult         = 2.6;

//--- TP2 / Runner control (New)
input bool                InpEnableTP2           = true;   // if false: no TP2, use trailing/opp-signal exits
...
input double InpTrailStart_RR=1.00;    // start trailing after 1.0R
...
input double InpTrail_ATR_Mult=1.80;
...
input double InpTP1_ClosePercent=15.0;    // close % at TP1

//--- Breakeven
input bool                InpEnableBE            = true;
input int                 InpBE_TriggerPoints    = 360;     // rekomendasi >= 2x spread
input int InpBE_OffsetPoints=20;      // BE + buffer
...
input int InpD_MaxATRPoints=0; // 0=off; jika >0 maka ENTRY di-skip bila ATRPoints melebihi nilai ini
//============================= GLOBALS =============================//
...
bool ModifyAllowedByFreeze(bool isBuy, double newSL, double newTP)
{
   ...
   double p = Pnt();
   double minDist = (double)freeze * p;

   // Reference price for freeze-level checks (use current bid/ask)
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double ref = isBuy ? bid : ask;

   // If prices are not available in tester yet, don't block modifications
   if(ref <= 0.0) return true;

   if(newSL > 0.0)
   {
      if(MathAbs(ref - newSL) < minDist) return false;
   }
   if(newTP > 0.0)
   {
      if(MathAbs(newTP - ref) < minDist) return false;
   }
   return true;
}
...
bool CalcSLTP(bool isBuy, double &slPrice, double &tpPrice)
{
   double p = Pnt();
   int digits = Dig();
   ...
   if(InpMaxSLPoints>0 && slPts > (double)InpMaxSLPoints) { PrintFormat("Reject: SLPoints=%.0f > MaxSLPoints=%d", slPts, InpMaxSLPoints); return false; }
...
bool SignalD(bool &buy, bool &sell)
{
   ...
   if(InpD_MaxATRPoints>0 && atrPts > (double)InpD_MaxATRPoints) return true; // filter ATR terlalu tinggi
...
void ApplyTrailing()
{
   ...
   // ------------------------------------------------------------
   // Trailing start gate:
   // - If InpTrailStart_RR > 0 and we know 1R (g_RPoints), start trailing only
   //   when the position has moved at least InpTrailStart_RR * 1R.
   // - Otherwise, if InpTrailStartPoints > 0, start trailing only after that
   //   many points in profit.
   // Without this gate, trailing may activate immediately after TP1 and can
   // cut big winners too early (especially on XAU).
   // ------------------------------------------------------------
