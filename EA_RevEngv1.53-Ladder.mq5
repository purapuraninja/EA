...
input bool   InpUseRecovery         = true;   // Aktifkan recovery grid
input bool   InpRecoveryTrendGate   = true;   // Stop ladder recovery bila tren melawan arahnya
input double InpRecoveryPosTarget   = 0.50;   // Target profit per POSISI recovery (USD harga, 0=off)
input double InpRecoveryTrigger     = 2500.0; // Picu saat floating basket <= -ini
...
input double InpRecoveryMinAgeHours = 2.0;    // Picu juga bila basket lebih tua dari ini (jam, 0=off)
input double InpRecoveryAgeFloatPct = 2.0;    // Syarat floating untuk jalur umur (% ekuitas)
input long   InpMagicRecovBuy       = 20264;  // Magic recovery arah BUY
input long   InpMagicRecovSell      = 20265;  // Magic recovery arah SELL

input group  "=== Dompet Laba Grid Utama (tebus posisi tua) ==="
// Kenapa ini ada, dari MC-v1.53-C.xlsx (akun 434119125, margin call 21 Agt):
//
//   Basket SELL berumur 29 jam menanggung -18.213 dari total -18.947. 31
//   posisi berumur >= 8 jam (0,7% dari 4.233 posisi, 1,74 lot) menyumbang
//   90,1% seluruh kerugian; 4.202 posisi sisanya net hanya -1.876.
//
//   Cicilan recovery tidak mungkin menyusul. Satu siklus recovery bersih
//   3,50 USD (target 1,00 x 0,05 lot, spread 0,30). Pendalaman floating =
//   net lot x money-per-USD x laju harga: pada 3,84 lot net dan laju
//   0,084 USD/menit itu 1.935 USD/jam, butuh 9,22 siklus/MENIT hanya untuk
//   imbang. Terukur di laporan: cicilan menang 4 dari 18 setengah-jam.
//
//   Yang bekerja adalah menebus posisi tua memakai laba yang SUDAH masuk,
//   selagi masih murah. Posisi yang sama berbiaya 317 USD pada 03.30 tapi
//   595 USD pada 12.11; menebus seluruh ekor 29 posisi berbiaya 471 USD
//   pada 01.00 dan 16.175 USD pada 16.00. Utang ini berbunga.
//
//   Simulasi pada laporan yang sama, mulai saat trigger 2500 menyala,
//   prioritas TERTUA dulu (dana = persen laba basket yang tutup untung):
//     dana  30% -> 17 tebus, floating 16.20 -14.490, net 6,75 lot
//     dana  50% -> 22 tebus, floating 16.20 -10.394, net 6,32 lot
//     dana  70% -> 27 tebus, floating 16.20  -5.479, net 5,77 lot
//   Aktual tanpa tebusan: floating -18.213, net 7,71 lot.
//   Laba yang hilang NOL sampai dana 70%: semua posisi tua yang ditebus
//   memang berakhir rugi juga, bukan posisi yang akan pulih.
//
//   Ini BUKAN cut loss: tidak ada SL dipasang, tidak ada ambang harga,
//   basket tidak pernah ditutup utuh, dan dananya hanya laba yang sudah
//   terealisasi - bukan modal. Sisa basket tetap dibiarkan pulih sendiri.
input bool   InpUseProfitWallet     = true;   // Laba grid utama ikut mendanai tebusan posisi tua
input double InpProfitWalletPct     = 50.0;   // Porsi laba grid utama yang masuk dompet (%)
input double InpProfitWalletMinAgeH = 2.0;    // Umur minimum posisi yang boleh ditebus (jam)
input bool   InpSpendOldestFirst    = true;   // Prioritas tebus: TERTUA dulu (false = paling rugi dulu)

input group  "=== Stop Ladder Global (floating BUY+SELL) ==="
// Kenapa ini ada, dari replay lintas tujuh laporan 23 Agustus 2026:
//
//   Pemicu recovery lama dihitung PER basket. Pada contoh nyata BUY -1500
//   dan SELL -1000, tidak ada satu pun sisi yang menembus -2500, jadi
//   ladder kedua sisi tetap mengisi walau akun sudah menanggung -2500.
//
//   Replay counterfactual (C:\tmp\replay_global_recovery.py) menghentikan
//   SEMUA fill grid utama begitu floating gabungan menembus -2500. Volume
//   fill yang tidak lagi terjadi, per laporan:
//     MC-v1.53    257,06 lot     MC-v1.53-A  118,10 lot
//     MC-v1.53-B  571,64 lot     MC-v1.53-C   75,20 lot
//     Backtest     36,47 lot     Backtest-1   50,38 lot
//   New-v1.53 tidak pernah menembus ambang, jadi laporan sehat itu sama
//   sekali tidak tersentuh kebijakan ini.
//
//   Yang dihentikan hanya PENAMBAHAN eksposur grid utama. Yang TIDAK
//   dilakukan: tidak ada posisi ditutup, tidak ada SL dipasang di wilayah
//   rugi, tidak ada cut loss. Basket yang sudah terbuka tetap dikelola
//   trailing dan exitnya, dan jalur recovery justru dibuka supaya floating
//   dikurangi memakai laba yang benar-benar terealisasi.
//
//   Batas bukti yang HARUS diingat: XLSX tidak menyimpan magic dan tidak
//   memuat tick, jadi replay itu menguji geometri eksposur, BUKAN bukti
//   akun selamat. Backtest exact-tick dengan Journal-lah yang menentukan.
input bool   InpUseGlobalStop       = true;   // Stop ladder grid utama saat floating BUY+SELL menembus ambang
input double InpGlobalStopTrigger   = 2500.0; // Ambang floating gabungan BUY+SELL (mata uang akun)
input double InpGlobalStopReleasePct = 50.0;  // Lepas stop saat floating gabungan pulih ke % ambang ini
input bool   InpGlobalStopFeedsRecov = true;  // Saat stop aktif, basket minus mana pun boleh dapat recovery
...
// Stop ladder global. Dilatch supaya tidak berkedip: nyala saat floating
// gabungan BUY+SELL grid utama menembus -InpGlobalStopTrigger, dan hanya
// lepas setelah floating pulih ke persentase ambang yang ditetapkan.
// Hanya menahan PENAMBAHAN eksposur grid utama; tidak menutup apa pun.
bool     g_global_stop_block         = false;
double   g_global_stop_float         = 0.0;
datetime g_global_stop_since         = 0;
...
int OnInit()
{
   ...
   if(InpUseRecovery)
   {
      ...
      if(InpRecoveryMinAgeHours < 0.0)
      {
         Print("InpRecoveryMinAgeHours tidak boleh negatif "
               "(0 = matikan jalur umur).");
         return INIT_PARAMETERS_INCORRECT;
      }
      if(InpRecoveryMinAgeHours > 0.0 &&
         (InpRecoveryAgeFloatPct <= 0.0 || InpRecoveryAgeFloatPct >= 100.0))
      {
         Print("InpRecoveryAgeFloatPct harus antara 0 dan 100 bila "
               "jalur umur aktif.");
         return INIT_PARAMETERS_INCORRECT;
      }
   ...
   if(InpUseGlobalStop)
   {
      if(InpGlobalStopTrigger <= 0.0)
      {
         Print("InpGlobalStopTrigger harus > 0. Matikan lewat "
               "InpUseGlobalStop kalau tidak dipakai.");
         return INIT_PARAMETERS_INCORRECT;
      }
      // Release 100% berarti stop lepas tepat di ambang, dan itu membuat
      // status berkedip: satu tick blok, tick berikutnya refill enam
      // pending di titik terburuk. Histeresis wajib benar-benar ada.
      if(InpGlobalStopReleasePct <= 0.0 || InpGlobalStopReleasePct >= 100.0)
      {
         Print("InpGlobalStopReleasePct harus di antara 0 dan 100 "
               "(eksklusif). Nilai 100 menghapus histeresis dan membuat "
               "stop berkedip di sekitar ambang.");
         return INIT_PARAMETERS_INCORRECT;
      }
      if(InpGlobalStopFeedsRecov && !InpUseRecovery)
         Print("PERINGATAN: InpGlobalStopFeedsRecov aktif tapi InpUseRecovery "
               "mati. Ladder akan berhenti saat ambang tersentuh tanpa ada "
               "mesin yang mengurangi floating - basket hanya menunggu harga "
               "kembali sendiri.");
      PrintFormat("Stop ladder global AKTIF: ambang floating gabungan "
                  "BUY+SELL -%.2f, lepas di -%.2f (%.0f%%). Recovery saat "
                  "stop: %s. Tidak ada posisi ditutup dan tidak ada SL rugi.",
                  InpGlobalStopTrigger,
                  InpGlobalStopTrigger * InpGlobalStopReleasePct / 100.0,
                  InpGlobalStopReleasePct,
                  InpGlobalStopFeedsRecov ? "semua basket minus dilayani"
                                          : "hanya ambang lama");
   }

   if(InpUseProfitWallet)
   {
      if(InpProfitWalletPct <= 0.0 || InpProfitWalletPct > 100.0)
      {
         Print("InpProfitWalletPct harus di antara 0 dan 100. "
               "Matikan lewat InpUseProfitWallet kalau tidak dipakai.");
         return INIT_PARAMETERS_INCORRECT;
      }
      if(InpProfitWalletMinAgeH < 0.0)
      {
         Print("InpProfitWalletMinAgeH tidak boleh negatif.");
         return INIT_PARAMETERS_INCORRECT;
      }
      if(!InpUseRecovery)
         Print("PERINGATAN: InpUseProfitWallet aktif tapi InpUseRecovery mati. "
               "Dompet laba tetap bekerja menebus posisi tua grid utama, "
               "hanya saja tidak ada basket recovery yang ikut mendanainya.");
   }
...
void OnTick()
{
   ...
   // Stop ladder global dihitung SEBELUM ManageGrid, karena MaintainLadder
   // dan izin anchor keduanya membacanya pada tick yang sama.
   bool global_stop = UpdateGlobalStop();
   ...
   // Sisa dompet laba yang belum terpakai dicoba dibelanjakan tiap tick.
   // Perlu karena posisi bisa BARU memenuhi syarat umur setelah dompetnya
   // terisi, dan tanpa ini sisa dompet menganggur sampai ada basket tutup
   // berikutnya. Murah: langsung keluar kalau dompetnya kosong.
   TopUpFromProfitWallet(InpMagicBuy);
   TopUpFromProfitWallet(InpMagicSell);
   ...
   string state = "AKTIF";
   if(global_stop)
      state = StringFormat("STOP LADDER GLOBAL: floating gabungan %.2f, "
                           "ladder utama berhenti, recovery jalan, "
                           "TANPA CUT LOSS", g_global_stop_float);
   else if(floating_block)
...
//| STOP LADDER GLOBAL.                                              |
//|                                                                  |
//| Pemicu recovery lama dihitung per basket, jadi kombinasi seperti   |
//| BUY -1500 dan SELL -1000 tidak pernah menembus ambang -2500 walau  |
//| akun sudah menanggung -2500. Fungsi ini menutup celah itu: yang    |
//| dibaca adalah floating GABUNGAN kedua basket grid utama.           |
//|                                                                  |
//| Yang dilakukan saat aktif: TIDAK ada level ladder baru dan pending |
//| yang belum terisi dibatalkan, sehingga eksposur berhenti bertambah.|
//| Yang TIDAK dilakukan: tidak ada posisi ditutup, tidak ada SL di    |
//| wilayah rugi, tidak ada cut loss. Basket yang sudah ada tetap      |
//| dikelola trailing dan exitnya seperti biasa.                      |
//|                                                                  |
//| Dilatch dengan histeresis. Tanpa itu status berkedip di sekitar    |
//| ambang: satu tick blok lalu tick berikutnya refill enam pending,   |
//| yang justru menambah eksposur tepat di titik terburuk. Stop baru   |
//| lepas setelah floating gabungan pulih ke InpGlobalStopReleasePct   |
//| dari ambang - default 50%, jadi -2500 baru lepas di -1250.         |
//|                                                                  |
//| Recovery TIDAK ikut diblokir. Justru sebaliknya: selama stop aktif |
//| jalur recovery adalah satu-satunya mesin yang boleh mengurangi     |
//| floating, memakai laba yang benar-benar sudah terealisasi.         |
//+------------------------------------------------------------------+
bool UpdateGlobalStop()
{
   if(!InpUseGlobalStop)
   {
      g_global_stop_block = false;
      g_global_stop_float = 0.0;
      g_global_stop_since = 0;
      return false;
   }

   // Hanya grid UTAMA yang dihitung. Floating basket recovery sengaja tidak
   // ikut: recovery justru sedang bekerja memperbaiki keadaan, dan kalau
   // floatingnya ikut dijumlahkan, recovery yang sehat bisa menahan dirinya
   // sendiri atau melepas stop terlalu cepat.
   double flt = BasketFloating(InpMagicBuy) + BasketFloating(InpMagicSell);
   g_global_stop_float = flt;

   double trigger = -InpGlobalStopTrigger;
   double release = -InpGlobalStopTrigger * InpGlobalStopReleasePct / 100.0;

   if(!g_global_stop_block)
   {
      if(flt <= trigger)
      {
         g_global_stop_block = true;
         g_global_stop_since = TimeCurrent();
         PrintFormat("STOP LADDER GLOBAL AKTIF: floating gabungan %.2f <= "
                     "%.2f (BUY %.2f + SELL %.2f). Ladder grid utama "
                     "dihentikan dan pending dibatalkan. TANPA CUT LOSS: "
                     "%d posisi BUY dan %d posisi SELL tetap dipegang, "
                     "trailing dan exit tetap jalan. Lepas di %.2f.",
                     flt, trigger,
                     BasketFloating(InpMagicBuy), BasketFloating(InpMagicSell),
                     CountPositions(InpMagicBuy), CountPositions(InpMagicSell),
                     release);
      }
   }
   ...
   {
      long held = (long)TimeCurrent() - (long)g_global_stop_since;
      if(held < 0) held = 0;
      PrintFormat("STOP LADDER GLOBAL LEPAS: floating gabungan %.2f >= %.2f "
                  "setelah %.1f menit. Ladder grid utama dilanjutkan.",
                  flt, release, (double)held / 60.0);
      g_global_stop_block = false;
      g_global_stop_since = 0;
   }

   return g_global_stop_block;
}
...
// Dompet laba grid utama. Terpisah dari dompet recovery supaya keduanya
// bisa dibaca sendiri-sendiri di log dan panel: dompet recovery diisi dari
// laba basket recovery, dompet ini diisi dari laba basket grid UTAMA.
// Satu dompet per arah basket yang ditebus, bukan satu untuk seluruh akun,
// supaya laba basket BUY tidak dipakai menebus posisi BUY-nya sendiri.
double   g_profit_wallet[2]   = {0.0, 0.0};

string GVProfitWalletName(long magic) { return "MasTri_" + _Symbol + "_" + (string)magic + "_pwallet"; }

void SetProfitWallet(long target_magic, double v)
{
   int idx = DirIndex(DirForMagic(target_magic));
   g_profit_wallet[idx] = v;
   string gv = GVProfitWalletName(target_magic);
   if(v > 0.0) GlobalVariableSet(gv, v);
   else if(GlobalVariableCheck(gv)) GlobalVariableDel(gv);
}

double GetProfitWallet(long target_magic)
{
   int idx = DirIndex(DirForMagic(target_magic));
   if(g_profit_wallet[idx] <= 0.0)
   {
      string gv = GVProfitWalletName(target_magic);
      if(GlobalVariableCheck(gv))
         g_profit_wallet[idx] = GlobalVariableGet(gv);
   }
   return g_profit_wallet[idx];
}
...
bool OpenAnchor(ENUM_POSITION_TYPE dir, long magic)
{
   ...
   string cmt = IsRecoveryMagic(magic) ? "RECOV" : InpComment;
   bool ok;
   double price;
   if(dir == POSITION_TYPE_BUY)
   {
      price = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
      ok = trade.Buy(lot, _Symbol, 0.0, 0.0, 0.0, cmt);
   }
   else
   {
      price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      ok = trade.Sell(lot, _Symbol, 0.0, 0.0, 0.0, cmt);
   }
   if(ok)
   {
                       ...
                       int level, bool freeze_ramp)
{
   ...
   if(dir == POSITION_TYPE_BUY)
   {
      ...
      ok = trade.BuyLimit(lot, price, _Symbol, 0.0, 0.0, ORDER_TIME_GTC, 0, cmt);
   }
   else
   {
      price  = NormPrice(anchor + dist);
      ...
      ok = trade.SellLimit(lot, price, _Symbol, 0.0, 0.0, ORDER_TIME_GTC, 0, cmt);
   }
                    ...
                    double be, double price)
{
   ...
   // Stop ladder global: floating GABUNGAN BUY+SELL sudah menembus ambang.
   // Berlaku hanya untuk grid UTAMA - basket recovery justru harus tetap
   // boleh bekerja, karena ia satu-satunya yang mengurangi floating dengan
   // laba nyata. Pending dibatalkan supaya harga yang terus berjalan tidak
   // menambah posisi, dan tidak ada level baru dipasang. Posisi terbuka
   // TIDAK ditutup dan tidak diberi SL: trailing/exit tetap lewat
   // ApplyBasketStop() dan MaybeExit().
   if(g_global_stop_block && !IsRecoveryMagic(magic))
   {
      int pend_gs = CountPendings(magic);
      if(pend_gs > 0)
         CancelAllPendings(magic);
      static datetime s_gstop_log[BASKET_SLOTS] = {0, 0, 0, 0};
      int gslot = SlotOf(magic);
      if(TimeCurrent() - s_gstop_log[gslot] >= 60)
      {
         PrintFormat("STOP LADDER GLOBAL %s: floating gabungan %.2f, %d "
                     "pending dibatalkan, %d posisi tetap dipegang. "
                     "TANPA CUT LOSS.",
                     BasketName(magic), g_global_stop_float, pend_gs,
                     CountPositions(magic));
         s_gstop_log[gslot] = TimeCurrent();
      }
      return;
   }
...
//+------------------------------------------------------------------+
//| Umur basket dalam JAM, dihitung dari posisi PALING TUA.            |
//|                                                                  |
//| Dipakai sebagai pemicu recovery tambahan. Dasarnya pengukuran atas |
//| 3.337 basket dari tiga laporan nyata: basket berumur < 1 jam       |
//| nyaris tidak pernah merugi lebih dari -743, sementara SATU basket  |
//| berumur 4,54 jam menghabiskan satu akun (-29.207) dan tiga basket  |
//| berumur > 6 jam menyumbang -10.268 di akun lain. Persen floating   |
//| pembeda yang buruk: sebuah basket menyentuh 50% ekuitas hanya 43   |
//| menit setelah mulai lalu sembuh sendiri dan tutup +4.748. Umur     |
//| memisahkan basket yang sembuh dari basket yang membunuh.           |
//|                                                                  |
//| Ditegaskan lagi oleh MC-v1.53-C.xlsx: basket SELL yang membunuh    |
//| akun dibuka 20 Agt 11.48, tapi ambang floating -2.500 baru         |
//| tersentuh 21 Agt 03.00 - posisi tertua sudah berumur 15,2 jam      |
//| SEBELUM recovery menyala, karena basketnya cuma 1,64 lot. Ambang   |
//| berbasis mata uang tidak pernah melihat basket kecil-tapi-tua, dan |
//| justru itu jenis basket yang mematikan.                            |
//+------------------------------------------------------------------+
double BasketAgeHours(long magic)
{
   datetime oldest = 0;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
   ...
   long age = (long)TimeCurrent() - (long)oldest;
   if(age < 0) age = 0;         // jam server geser, jangan sampai negatif
   return (double)age / 3600.0;
}
...
//|                                                                  |
//| PRIORITAS TERTUA (InpSpendOldestFirst) - dari MC-v1.53-C.xlsx:       |
//|                                                                  |
//| "Paling rugi dulu" mengejar posisi TERDALAM, yang selalu yang paling |
//| BARU (baru terisi karena harga baru sampai ke situ). Posisi tua yang |
//| justru menahan basket tetap tertinggal. Simulasi pada laporan itu,   |
//| dana 50% laba grid utama:                                          |
//|   prioritas TERTUA  -> 22 tebus, floating 16.20 -10.394, laba hilang 0 |
//|   prioritas TERMURAH-> 30 tebus, floating 16.20  -8.460, laba hilang 20|
//| Termurah menutup lebih banyak posisi tapi meninggalkan ekor tua yang  |
//| biayanya naik terus: satu posisi 317 USD pada 03.30 jadi 595 USD pada |
//| 12.11. Menebus tua lebih dulu memutus bunga majemuk itu.             |
//|                                                                  |
//| min_age_h > 0 menyaring posisi muda. Dipakai jalur dompet laba grid  |
//| utama; jalur recovery memakai 0 supaya perilakunya tidak berubah.    |
//+------------------------------------------------------------------+
double SpendRecoveryProfit(long target_magic, double budget,
                           double min_age_h = 0.0)
{
   ...
   for(int guard = 0; guard < 500; guard++)
   {
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         ...
         // Umur posisi. Jam server bisa bergeser: jangan sampai negatif.
         long asec = (long)now_srv -
                     (long)PositionGetInteger(POSITION_TIME);
         if(asec < 0) asec = 0;
         double age_h = (double)asec / 3600.0;
         if(min_age_h > 0.0 && age_h < min_age_h) continue;
         ...
         if(InpSpendOldestFirst)
         {
            // Tertua dulu. Seri umur diputus oleh yang lebih rugi.
            if(pick_ticket == 0 || age_h > pick_age ||
               (age_h == pick_age && pl < pick_pl))
            {
               pick_ticket = t;
               pick_pl     = pl;
               pick_age    = age_h;
         ...
         else if(pick_ticket == 0 || pl < pick_pl)
         {
            // Bentuk lama: yang paling rugi. Penurunan floating terbesar
            // sekaligus perbaikan BE terbesar.
            pick_ticket = t;
            pick_pl     = pl;
            pick_age    = age_h;
         }
...
bool RecoveryAllowed(ENUM_POSITION_TYPE losing_dir)
{
   ...
   // ---------------- DUA JALUR PEMICU ----------------
   //
   // Jalur FLOATING (lama): floating basket menembus ambang mutlak.
   // Satuannya mata uang akun, jadi artinya berubah drastis antar akun.
   //
   // Jalur UMUR: basket lebih tua dari ambang DAN floatingnya minus
   // melebihi persentase kecil dari ekuitas. Karena berbasis persen, jalur
   // ini berlaku sama di akun ukuran apa pun tanpa disetel ulang.
   //
   // Lihat komentar BasketAgeHours() untuk data yang mendasarinya.
   bool hit_float = (lflt <= -InpRecoveryTrigger);
   ...
   if(InpRecoveryMinAgeHours > 0.0)
   {
      age_h = BasketAgeHours(lmagic);
      double eq = AccountInfoDouble(ACCOUNT_EQUITY);
      if(eq > 0.0)
      {
         age_floor = eq * InpRecoveryAgeFloatPct / 100.0;
         hit_age   = (age_h >= InpRecoveryMinAgeHours && lflt <= -age_floor);
      }
   }

   // Jalur STOP GLOBAL. Saat stop ladder global aktif, ladder grid utama
   // sudah dihentikan sehingga floating tidak lagi bertambah dari fill
   // baru - tapi floating yang SUDAH ada tetap harus dikurangi. Basket
   // minus mana pun boleh dilayani recovery pada fase ini, walau sisi itu
   // sendiri belum menembus ambang per-basket.
   //
   // Inilah yang menutup kasus BUY -1500 dan SELL -1000: gabungannya sudah
   // -2500, jadi sisi yang minus berhak dilayani tanpa menunggu salah satu
   // sisi sendirian mencapai -2500.
   bool hit_global = (InpUseGlobalStop && InpGlobalStopFeedsRecov &&
                      g_global_stop_block && lflt < 0.0);

   if(!hit_float && !hit_age && !hit_global)
      return false;
   ...
   if(g_recov_base[idx] <= 0.0)
   {
      ...
      double spread = SymbolInfoDouble(_Symbol, SYMBOL_ASK) -
                      SymbolInfoDouble(_Symbol, SYMBOL_BID);
      // Jalur mana yang menyalakan episode ini ikut dicetak, supaya
      // terlihat apakah jalur UMUR benar-benar bekerja di lapangan atau
      // jalur FLOATING lama yang tetap mendominasi.
      string why = "";
      if(hit_age)    why  = "UMUR";
      if(hit_float)  why += (StringLen(why) > 0) ? "+FLOATING" : "FLOATING";
      if(hit_global) why += (StringLen(why) > 0) ? "+STOP GLOBAL"
                                                : "STOP GLOBAL";

      PrintFormat("RECOVERY DIPICU untuk %s [jalur %s]: umur %.2f jam, "
                  "floating %.2f (ambang floating -%.2f, ambang umur "
                  "%.2f jam & -%.2f). Basket %.2f lot. Recovery %s "
                  "lot dasar %.2f, target profit %.2f USD.",
                  BasketName(lmagic), why, age_h, lflt,
                  InpRecoveryTrigger, InpRecoveryMinAgeHours, age_floor,
                  lvol, BasketName(rmagic), b, InpRecoveryTarget);
...
//+------------------------------------------------------------------+
//| DOMPET LABA GRID UTAMA: setor lalu belanjakan.                     |
//|                                                                  |
//| Dipanggil saat basket grid UTAMA tutup untung. Porsi labanya masuk  |
//| dompet arah SEBERANG, lalu dompet itu dipakai menebus posisi tua di |
//| basket seberang yang masih minus.                                  |
//|                                                                  |
//| Kenapa arah seberang: laba basket BUY tidak boleh dipakai menebus   |
//| posisi BUY lain, karena basket BUY baru saja tutup - posisi yang     |
//| tersisa dan menumpuk selalu ada di arah yang berlawanan. Itu pula   |
//| yang terjadi di MC-v1.53-C: basket BUY tutup untung berulang kali   |
//| sepanjang 21 Agt sementara basket SELL menua 29 jam tanpa disentuh. |
//|                                                                  |
//| Ini TIDAK memasang SL dan TIDAK memakai modal. Dananya laba yang    |
//| sudah terealisasi. Sisa basket dibiarkan pulih sendiri seperti      |
//| semula. Kalau tidak ada posisi tua yang layak, dompet disimpan.     |
//+------------------------------------------------------------------+
void FundProfitWallet(long closed_magic, double realized)
{
   if(!InpUseProfitWallet)      return;
   if(realized <= 0.0)          return;
   if(IsRecoveryMagic(closed_magic)) return;   // laba recovery punya jalurnya

   // Arah seberang: basket yang posisinya masih menumpuk.
   long target_magic = (closed_magic == InpMagicBuy) ? InpMagicSell
                                                    : InpMagicBuy;
   if(CountPositions(target_magic) == 0) return;
   // Basket seberang sedang dilikuidasi utuh: jangan ikut campur di tengah
   // likuidasi, sama seperti aturan CloseRecoveryWinners.
   if(g_closing[SlotOf(target_magic)]) return;

   double add    = realized * InpProfitWalletPct / 100.0;
   double wallet = GetProfitWallet(target_magic) + add;

   PrintFormat("DOMPET LABA: %s tutup untung %.2f, %.0f%% = %.2f masuk dompet "
               "%s. Dompet jadi %.2f. Menebus posisi umur >= %.1f jam, "
               "%s dulu. TANPA CUT LOSS - dana dari laba, bukan modal.",
               BasketName(closed_magic), realized, InpProfitWalletPct, add,
               BasketName(target_magic), wallet, InpProfitWalletMinAgeH,
               InpSpendOldestFirst ? "TERTUA" : "PALING RUGI");

   double used = SpendRecoveryProfit(target_magic, wallet,
                                     InpProfitWalletMinAgeH);
   wallet -= used;
   SetProfitWallet(target_magic, MathMax(0.0, wallet));
}

//+------------------------------------------------------------------+
//| MENUTUP KEKURANGAN DOMPET RECOVERY.                                |
//|                                                                  |
//| Dipanggil setelah dompet recovery selesai dibelanjakan. Kalau masih |
//| ada posisi tua yang belum tertebus - biasanya karena satu siklus    |
//| recovery bersih hanya 3,50 USD sementara posisi tua berbiaya 200    |
//| sampai 600 USD - sisa kebutuhannya diambil dari dompet laba grid    |
//| utama.                                                            |
//|                                                                  |
//| Aritmatika yang mendasarinya, dari MC-v1.53-C.xlsx: pendalaman      |
//| floating = net lot x money-per-USD x laju harga. Pada 3,84 lot net  |
//| dan 0,084 USD/menit itu 1.935 USD/jam, sedangkan satu siklus        |
//| recovery bersih 3,50 USD - butuh 9,22 siklus/MENIT hanya untuk      |
//| imbang, padahal satu siklus perlu harga bergerak 1,00 USD bolak     |
//| balik (12 menit pada laju itu). Cicilan recovery memang tidak       |
//| mungkin menyusul sendiri; kekurangannya harus ditambal laba grid.   |
//+------------------------------------------------------------------+
void TopUpFromProfitWallet(long target_magic)
{
   if(!InpUseProfitWallet) return;
   if(CountPositions(target_magic) == 0) return;
   if(g_closing[SlotOf(target_magic)]) return;   // sedang dilikuidasi utuh

   double wallet = GetProfitWallet(target_magic);
   if(wallet <= 0.0) return;

   double used = SpendRecoveryProfit(target_magic, wallet,
                                     InpProfitWalletMinAgeH);
   if(used <= 0.0) return;

   PrintFormat("DOMPET LABA menambal kekurangan dompet recovery %s: "
               "terpakai %.2f dari %.2f. Sisa dompet %.2f.",
               BasketName(target_magic), used, wallet, wallet - used);
   SetProfitWallet(target_magic, MathMax(0.0, wallet - used));
}
...
void RunClose(ENUM_POSITION_TYPE dir, long magic)
{
   ...
   // Sama untuk grid UTAMA: floating sesaat sebelum tutup adalah laba yang
   // akan direalisasi, dan sebagiannya masuk dompet laba. Wajib dibaca
   // SEBELUM CloseBasket() dengan alasan yang sama.
   double main_profit = 0.0;
   if(!IsRecoveryMagic(magic) && InpUseProfitWallet)
      main_profit = BasketFloating(magic);
   ...
   if(spend_target != 0)
   {
      ...
      // Dompet recovery sudah dibelanjakan sebisanya. Sisa posisi tua yang
      // belum tertebus ditambal dari dompet laba grid utama.
      TopUpFromProfitWallet(spend_target);
   }

   // Basket grid UTAMA tutup untung: porsi labanya mendanai tebusan posisi
   // tua di basket seberang. Inilah yang di MC-v1.53-C tidak pernah terjadi.
   if(main_profit > 0.0)
      FundProfitWallet(magic, main_profit);

   // Basket ini sudah kosong, jadi dompet yang diperuntukkan menebus
   // POSISINYA tidak ada lagi gunanya. Dilepas supaya tidak menggantung di
   // GlobalVariable dan tidak dipakai siklus berikutnya yang belum tentu
   // punya posisi tua. Uangnya tetap di balance, tidak hilang.
   if(!IsRecoveryMagic(magic) && GetProfitWallet(magic) > 0.0)
   {
      PrintFormat("DOMPET LABA %s dilepas %.2f: basket sudah kosong, "
                  "tidak ada posisi tua yang perlu ditebus.",
                  BasketName(magic), GetProfitWallet(magic));
      SetProfitWallet(magic, 0.0);
   }
...
void CloseRecoveryWinners(long rmagic)
{
   ...
   // Sisa posisi tua yang belum tertebus ditambal dari dompet laba grid
   // utama, sama seperti pada jalur RunClose.
   TopUpFromProfitWallet(spend_target);
}
